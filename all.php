...
// Manejamos las operaciones de edición y eliminación
if (isset($_POST['update'])) {
    $id = $_POST['id'];
    $estado = $_POST['estado'];
    $sql_update = "UPDATE clientes SET estado='$estado' WHERE id_number='$id'";
    mysqli_query($conexion, $sql_update);
}

if (isset($_POST['delete'])) {
    $id = $_POST['id'];
    $sql_delete = "DELETE FROM clientes WHERE id_number='$id'";
...
// Consulta SQL para seleccionar los registros de la tabla 'clientes' paginados y clasificados
$sql = "SELECT fecha, nombre, nacionalidad, caso, estado, id_number, nacimiento FROM clientes ORDER BY fecha, nombre, nacionalidad, caso LIMIT $empezar_desde, $por_pagina";
...
if ($resultado && mysqli_num_rows($resultado) > 0) {
    ...
    while ($fila = mysqli_fetch_assoc($resultado)) {
        ...
        echo "<td>" . $fila['id_number'] . "</td>";
        echo "<td>" . $fila['nacimiento'] . "</td>";
        echo "<td>";
        echo "<select name='estado'>";
        echo "<option value='Active &#x1F7E2;'" . ($fila['estado'] == 'Active' ? ' selected' : '') . ">Active</option>";
        echo "<option value='Inactive &#x1F534;'" . ($fila['estado'] == 'Inactive' ? ' selected' : '') . ">Inactive</option>";
        echo "<option value='Pending &#x1F7E2;'" . ($fila['estado'] == 'Pending' ? ' selected' : '') . ">Pending</option>";
        echo "</select>";
        echo "</td>";
        echo "<td>";
        echo "<input type='hidden' name='id' value='" . $fila['id_number'] . "' />";
        echo "<button type='submit' name='update'>Update</button>";
        echo "<button type='submit' name='delete' onclick='return confirm(\"Are you sure you want to delete this user?\")'>Delete</button>";
        echo "</td>";
        echo "</tr>";
    }
...
// Paginación
$sql_total = "SELECT COUNT(*) as total_filas FROM clientes";
...
for ($i=1; $i<=$total_paginas; $i++) {
    if ($i == $pagina) {
        ...
        echo "<a href='all.php?pagina=" . $i . "'>" . $i . "</a>";
    }
}
echo "</div>";
