...
function eliminarCliente($conexion, $identify) {
    // Consulta SQL para eliminar el cliente por su identificador
    $sql = "DELETE FROM portafolio WHERE identify = '$identify'";
    
    // Ejecuta la consulta
    if (mysqli_query($conexion, $sql)) {
        return true; // Éxito al eliminar el cliente
    } else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["delete"])) {
    // Verifica si se recibió el identificador del cliente
    if (isset($_POST["identify"])) {
        $identify = $_POST["identify"];
        // Intenta eliminar el cliente
        if (eliminarCliente($conexion, $identify)) {
...
if ($resultado && mysqli_num_rows($resultado) > 0) {
    ...
    while ($fila = mysqli_fetch_assoc($resultado)) {
        ...
        echo "<td>";
        echo "<form action='" . $_SERVER['PHP_SELF'] . "' method='post' onsubmit='return confirm(\"Are you sure you want to delete this case?\")'>";
        echo "<input type='hidden' name='identify' value='" . $fila['identify'] . "'>";
...
for ($i=1; $i<=$total_paginas; $i++) {
    if ($i == $pagina) {
        ...
        echo "<a href='" . $_SERVER['PHP_SELF'] . "?otra_pagina=" . $i . "'>" . $i . "</a>";
    }
}
echo "</div>";
